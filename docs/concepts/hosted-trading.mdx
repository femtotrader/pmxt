...
Self-hosted runs `pmxt-core` on your own machine; the SDK talks to `localhost`. Both modes expose the same SDK surface — the difference is where execution happens and who holds keys.
...
Choose self-hosted when you need **sub-100ms latency**, want to use **raw venue credentials** (Polymarket L2 API keys, Kalshi RSA, Smarkets sessions), have **regulatory requirements** that mandate direct counterparty status with the venue, or want to **trade on venues not yet supported by hosted**. See [Self-hosted](/guides/self-hosted) for setup.
