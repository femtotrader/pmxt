...
description: "The hosted-default execution path — how PMXT routes orders, why funds sit in the non-custodial PreFundedEscrow, and when to choose self-host instead."
---
...
2. **`trade.pmxt.dev`** — PMXT's hosted trading API. Routes orders, operates against the escrow contract under user-signed authorization, talks to the venue.
...
For the deployed contract addresses, admin model, audit status, and the raw on-chain calls that get your USDC out without PMXT's server, see [Hosted Custody Model](/concepts/hosted-custody).
