---
title: SQL
description: "Historical data backend for your app's charts, dashboards, and analytics."
---

The `/v0/sql` endpoint is a read-only SQL interface over PMXT's historical
orderbook data — the backend you point your app's charts, dashboards, and
post-hoc analytics at. Today it covers **Polymarket** (BBO updates, trades,
and full book snapshots) stored in ClickHouse.
...
The bread-and-butter query for an in-app price chart. Pull hourly candles
for a single outcome over the last week — drop the result straight into
Recharts, lightweight-charts, or whatever you render with. Filter by
`asset_id` to pick the Yes or No side.
